...
public class TextArchitect 
{
 ...
 public Coroutine buildProcess = null;
    ...
    public TextArchitect(Text dialoguetext, MonoBehaviour owner)
   {
      
        if (dialoguetext != null)
        {
            Text_UI = dialoguetext;
        }
        else
        {
            Debug.LogError("Dialoguetext is null! Assign a valid Text component.");
        }

        coroutineOwner = owner;

        
        if (Text_UI == null && Text_world == null)
        {
            Debug.LogError("Both Text_UI and Text_world are null! Ensure you assign one of them.");
        }
    }
...
public Coroutine Build(string text)
    {
        ...
        targettext = text;
        Stop();
            buildProcess = coroutineOwner.StartCoroutine(Building());
        return buildProcess;
    }
    ...
    public void Stop()
    {
        if (!isBuilding)
            return;
        coroutineOwner.StopCoroutine(buildProcess);
        buildProcess = null;
